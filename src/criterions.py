...
class SharpeCriterion(nn.Module):
    ...
    def forward(self, positions:torch.Tensor, inputs:dict) -> Tuple[torch.Tensor, dict]:
        # `positions` is a BxTxA tensor containing the amount of stock held
        ...
        # case scenario
        price = inputs['price']     # Bx(T+1)xA     Price at which stock is traded
        ...
        delta_pos = positions[:,1:,:] - positions[:,:-1,:]      # Bx(T+1)xA stock bought
        ...
        pnl =  pnl_t.sum(dim=1)            # BxA   pnl per candlestick
        pnl =  pnl.sum(dim=1)              # B   pnl per candlestick
        # sharpe = pnl.mean() / (pnl.std() + 1e-8)

        metrics = {
            # 'pnl_t': pnl_t,
            # 'pnl_cumsum': torch.cumsum(pnl_t, dim=1)[:,0],     # accummulated pnl over time
            'pnl_sum': pnl.sum().item(),
