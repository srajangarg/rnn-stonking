...
class TBVisualizer():
    ...
    def plot_images(self, images:dict, global_step:int) -> None:
    ...
    def plot_videos(self, videos:dict, global_step:int, fps:int=4) -> None:
    ...
    def plot_meshes(self, meshes:dict, global_step:int) -> None:
    ...
    def save_meshes(self, meshes:dict, global_step:int) -> None:
    ...
    def plot_embeddings(self, embeddings:dict, global_step:int) -> None:
        for label, embed in embeddings.items():
            if isinstance(embed,dict):
                ...
                self.viz.add_embedding(embed, tag=label, CVBNMglobal_step=global_step)

    def plot_histograms(self, histograms:dict, global_step:int) -> None:
    ...
    def plot_texts(self, texts:dict, global_step:int) -> None:
        for label, text in texts.items():
            self.viz.add_text(label,text,global_step=global_step)

    # |visuals|: dictionary of images to display or save
    def display_current_results(self, visuals:dict, global_step:int, save_meshes:bool=False) -> None:
        if 'img' in visuals:
    ...
    def save_raw_stats(self, stats:dict, name:str, epoch:int) -> None:
        path = f'{self.stats_dir}/{name}_{epoch}'
        np.savez(path, **stats)

    def log_histogram(self, log_dict:dict, global_step:int) -> None:
        for tag, value in log_dict.items():
            self.viz.add_histogram(tag, value.data.cpu().numpy(), global_step)
        return

    def plot_current_scalars(self, scalars:dict, global_step:int) -> None:
